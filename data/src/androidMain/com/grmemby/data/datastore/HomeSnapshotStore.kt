package com.grmemby.data.datastore
...
class HomeSnapshotStore(
    ...
    private val homeSnapshotFileName = "GrmembySnapshot.json"
    ...
    private fun readSnapshot(file: File): PersistedHomeSnapshot? {
        ...
        return runCatching {
            ...
            GrmembyJson.decodeFromString<PersistedHomeSnapshot>(rawJson)
        }.getOrElse {
            file.delete()
            null
        }
    ...
    private fun writeSnapshotAtomically(
        ...
        try {
            ...
            stream.write(GrmembyJson.encodeToString(snapshot).toByteArray(StandardCharsets.UTF_8))
            stream.flush()
