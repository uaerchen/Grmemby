...
import com.grmemby.data.model.SeerrPersonCreditType
import com.grmemby.data.model.SeerrRecommendationTitle
import com.grmemby.data.model.SeerrRequestState
...
class SeerrRepository(context: Context) {
                ...
                when {
                    creditsResponse.isSuccessful -> {
                        val payload = runCatching {
                            GrmembyJson.decodeFromString<SeerrCombinedCreditsResponse>(responseBody)
                        }.getOrNull()
            ...
            response.use { searchResponse ->
                ...
                when {
                    searchResponse.isSuccessful -> {
                        val payload = runCatching {
                            GrmembyJson.decodeFromString<SeerrSearchResponse>(responseBody)
                        }.getOrNull()
                            ?: return@use Result.failure(
    ...
    private fun performSignInRequest(
        ...
        val requestBody = GrmembyJson.encodeToString(
            SeerrLoginRequest(
                username = username,
                password = password,
    ...
    private fun getStatusPayload(
        ...
        return statusResponse.use { response ->
            ...
            when {
                response.isSuccessful -> {
                    val payload = runCatching {
                        GrmembyJson.decodeFromString<SeerrStatusResponse>(responseBody)
    ...
    private fun buildMediaServerUrl(serverUrl: String): Result<String> {
        val trimmed = serverUrl.trim()
        if (trimmed.isBlank()) {
            return Result.failure(Exception("Your media server URL is missing. Sign in to Grmemby first."))
        }
    ...
    private fun decodeCurrentUserId(responseBody: String): Int? {
        return runCatching {
            GrmembyJson.decodeFromString<SeerrCurrentUserResponse>(responseBody).id
        }.getOrNull()
    }
    ...
    private fun fetchEffectiveRequestLimits(
        ...
        return response.use { quotaResponse ->
            ...
            val payload = runCatching {
                GrmembyJson.decodeFromString<SeerrQuotaResponse>(
