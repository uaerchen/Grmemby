...
import com.grmemby.player.video.HardwareAcceleration
import com.grmemby.player.video.HdrCapabilityManager
...
@UnstableApi
object PlayerUtils {
    private const val ONE_MEBIBYTE = 1024L * 1024L
    private const val MAX_EXO_TARGET_BUFFER_MB = 192
    private const val MAX_EXO_BATTERY_TARGET_BUFFER_MB = 96
    private const val MIN_EXO_TARGET_BUFFER_MB = 50
    ...
    fun createStreamingMediaSource(
        ...
        val dataSourceFactory = PlayerCacheManager.createDataSourceFactory(
            ...
            diskCacheEnabled = playerPreferences.isPlayerDiskCacheEnabled(),
    ...
    private fun createLoadControl(
        ...
        val requestedCacheTimeMs = playerPreferences.getPlayerCacheTimeSeconds()
            .coerceIn(
        ...
        val warmBufferTargetMs = if (batteryOptimizationEnabled) 60_000 else 120_000
        val minBufferMs = minOf(requestedCacheTimeMs, warmBufferTargetMs)
            .coerceAtLeast(PlayerPreferences.MIN_PLAYER_CACHE_TIME_SECONDS * 1000)
        val maxBufferMs = requestedCacheTimeMs.coerceAtLeast(minBufferMs)
        val playbackBufferMs = minOf(if (batteryOptimizationEnabled) 1_500 else 1_000, minBufferMs)
        val rebufferPlaybackMs = minOf(if (batteryOptimizationEnabled) 4_000 else 2_000, maxBufferMs)
        val targetBufferBytes = safeTargetBufferBytes(
            context = context,
            requestedCacheSizeMb = playerPreferences.getPlayerCacheSizeMb(),
        ...
        return DefaultLoadControl.Builder()
            ...
            // When disk cache is enabled, the user-facing cache-time slider should control
            // the forward loading window. Keeping the byte cap authoritative made large
            // settings such as 5000 MB / 900 s stop at the legacy 96-192 MB in-memory guard.
            // If disk cache is disabled or battery mode is active, keep the heap guard hard.
            .setPrioritizeTimeOverSizeThresholds(diskCacheEnabled && !batteryOptimizationEnabled)
            .build()
    }

    private fun safeTargetBufferBytes(
        context: Context,
        requestedCacheSizeMb: Int,
        batteryOptimizationEnabled: Boolean
    ): Int {
        val requestedMb = requestedCacheSizeMb.coerceIn(
            PlayerPreferences.MIN_PLAYER_CACHE_SIZE_MB,
            PlayerPreferences.MAX_PLAYER_CACHE_SIZE_MB
        )
        val memoryClassMb = runCatching {
            context.applicationContext.getSystemService(ActivityManager::class.java)?.largeMemoryClass
        }.getOrNull()?.takeIf { it > 0 } ?: 256
        val heapBudgetMb = (memoryClassMb / if (batteryOptimizationEnabled) 6 else 4)
            .coerceAtLeast(MIN_EXO_TARGET_BUFFER_MB)
        val policyCapMb = minOf(
            if (batteryOptimizationEnabled) MAX_EXO_BATTERY_TARGET_BUFFER_MB else MAX_EXO_TARGET_BUFFER_MB,
            heapBudgetMb
        )
        val targetMb = requestedMb.coerceAtMost(policyCapMb).coerceAtLeast(MIN_EXO_TARGET_BUFFER_MB)
        return targetMb.toLong()
            .times(ONE_MEBIBYTE)
