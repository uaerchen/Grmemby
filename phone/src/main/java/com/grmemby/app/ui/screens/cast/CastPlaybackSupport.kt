package com.grmemby.app.ui.screens.cast
...
import com.grmemby.app.cast.CastPlaybackState
...
private fun buildCastTrackOptions(
    ...
    val labelPrefix = if (streamType.equals("Audio", ignoreCase = true)) "音轨" else "字幕"
    ...
    if (matchingStreams.isEmpty()) {
        return if (includeOffOption) listOf(CastTrackOption(label = "关闭", streamIndex = -1)) else emptyList()
    }
    ...
    return if (includeOffOption) {
        listOf(CastTrackOption(label = "关闭", streamIndex = -1)) + options
    } else {
        options
    }
}
