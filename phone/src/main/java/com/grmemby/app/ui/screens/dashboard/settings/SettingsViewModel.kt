...
import com.grmemby.data.repository.SeerrConnectionInfo
import com.grmemby.data.repository.SeerrUserRequestLimits
...
data class SettingsUiState(
    ...
    val cacheNextEpisodeEnabled: Boolean = PlayerPreferences.DEFAULT_CACHE_NEXT_EPISODE,
    ...
    val blockIpv6Connections: Boolean = NetworkPreferences.DEFAULT_BLOCK_IPV6_CONNECTIONS,
...
class SettingsViewModel(
    ...
    fun setBlockIpv6ConnectionsEnabled(enabled: Boolean) {
        networkPreferences.setBlockIpv6ConnectionsEnabled(enabled)
        _uiState.value = _uiState.value.copy(
            blockIpv6Connections = networkPreferences.isBlockIpv6ConnectionsEnabled()
        )
    }
        ...
        viewModelScope.launch {
            ...
            if (scopeId.isNullOrBlank()) {
                val result = Result.failure<SeerrConnectionInfo>(
                    Exception("No active Grmemby session is available.")
                )
                setSeerrState(
                    SeerrUiState(
